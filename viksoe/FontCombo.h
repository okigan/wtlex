...
public:
   ...
   static BOOL CALLBACK _EnumFamCB(CONST LOGFONT* lplf, CONST TEXTMETRIC* lptm, DWORD FontType, LPARAM pThis) 
   {
      ...
      if( (tm.tmPitchAndFamily & TMPF_TRUETYPE) != 0 ) {
         ...
         if( (lpntm->ntmFlags & 0x60000) != 0 ) tm.tmPitchAndFamily |= TMPF_OPENTYPE;
      }
   ...
   void MeasureItem(LPMEASUREITEMSTRUCT lpMIS)
   {
      ...
      if( (m_dwExtStyle & FPS_EX_SHOWFACE) != 0 ) {
         // Already initialized 'tm'...
      }
      else if( (m_dwExtStyle & FPS_EX_FIXEDBOLD) != 0 && ((tm.tmPitchAndFamily & 3) == FIXED_PITCH) ) {
   ...
   void DrawItem(LPDRAWITEMSTRUCT lpDIS)
   {
      ...
      bool bFocus = (lpDIS->itemState & ODS_FOCUS) != 0;

      // Fill background and Font item
      CBrushHandle brBack = ::GetSysColorBrush(bSelected ? COLOR_HIGHLIGHT : COLOR_WINDOW);
      dc.FillRect(&rcItem, brBack);

      RECT rcText = rcItem;
      ::InflateRect(&rcText, -s_cxIndent, 0);

      if( (m_dwExtStyle & FPS_EX_TYPEICON) != 0 ) 
      {
         if( (tm.tmPitchAndFamily & TMPF_OPENTYPE) != 0 ) {
            dc.DrawIconEx(rcText.left, rcText.top + 1, m_hOtIcon, 16, 16);
         }
         else if( (tm.tmPitchAndFamily & TMPF_TRUETYPE) != 0 ) {
            dc.DrawIconEx(rcText.left, rcText.top + 1, m_hTtIcon, 16, 16);
         }
         rcText.left += 18;
      }
      ...
      dc.DrawText(lf.lfFaceName, -1, &rcText, DT_SINGLELINE | DT_LEFT | DT_VCENTER | DT_NOPREFIX | DT_EDITCONTROL | DT_END_ELLIPSIS);
      if( hOldFont ) dc.SelectFont(hOldFont);
      if( bFocus ) dc.DrawFocusRect(&rcItem);
   }
};
